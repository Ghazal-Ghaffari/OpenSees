# Minimal Steel01 a2 sensitivity probe
...
# Cyclic load history: increase to +80, then reverse to -120
set loadValues {}

for {set p 0} {$p <= 80} {incr p} {
    lappend loadValues $p
}

for {set p 79} {$p >= -120} {incr p -1} {
    lappend loadValues $p
}
...
# DDM sensitivity setup for Steel01 parameter a2
reliability
parameter 1 element 1 a2
sensitivityIntegrator -static
sensitivityAlgorithm -computeAtEachStep

# Run the complete cyclic history
set nSteps [expr {[llength $loadValues] - 1}]
set ok [analyze $nSteps]

# Read final displacement and its DDM sensitivity to a2
set u [nodeDisp 2 1]
